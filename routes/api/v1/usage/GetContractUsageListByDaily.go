...
func GetContractUsageListByDaily(c *gin.Context) {
	url := os.Getenv("COST_URL")
	accessKey := os.Getenv("NCP_ACC")
	useStartDay := c.Query("useStartDay")
	useEndDay := c.Query("useEndDay")
	request := util.RequestInfo{
		Method: "GET",
		Path:   "/billing/v1/cost/getContractUsageListByDaily",
		Query:  fmt.Sprintf("?useStartDay=%s&useEndDay=%s&responseFormatType=json", useStartDay, useEndDay),
	}
