...
func GetProductPriceList(c *gin.Context) {
	url := os.Getenv("COST_URL")
	accessKey := os.Getenv("NCP_ACC")

	// Query 생성 로직
	query := "?"
	payCurrencyCode := c.Query("payCurrencyCode")
	if payCurrencyCode != "" {
		query = query + "payCurrencyCode=" + payCurrencyCode + "&"
	}
	...
	request := util.RequestInfo{
		...
		Query:  fmt.Sprintf("%s%s", query, "responseFormatType=json"),
	}
